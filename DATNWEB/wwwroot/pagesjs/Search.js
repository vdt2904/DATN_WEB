    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                table += '<div class="ep">' + (products[i].maxEpisode == -1 ? '??' : products[i].maxEpisode) + ' /' + (products[i].totalEpisode == null ? '??' : products[i].totalEpisode) + '</div>';
            ...
            renderPagination(a,response.paginationInfo);
        },
...
function renderPagination(a,paginationInfo) {
    let paginationHtml = '';

    // Add page links to pagination
    if (paginationInfo.totalPages > 1) {
        for (let i = 1; i <= paginationInfo.totalPages; i++) {
            paginationHtml += '<a href="#" onclick="searchs(\'' + a + '\', ' + i + ')">' + i + '</a>';

        }
    }
    // Display previous page link
    if (paginationInfo.currentPage > 1) {
        paginationHtml += '<a href="#" onclick="searchs(\'' + a + '\',' + (paginationInfo.currentPage - 1) + ')"><i class="fa fa-angle-double-left"></i></a>';
    }

    // Display next page link
    if (paginationInfo.currentPage < paginationInfo.totalPages) {
        paginationHtml += '<a href="#" onclick="searchs(\'' + a + '\',' + (paginationInfo.currentPage + 1) + ')"><i class="fa fa-angle-double-right"></i></a>';
    }
