...
function renderPagination(paginationInfo) {
    ...
    if (paginationInfo.totalPages > 1) {
        for (let i = 1; i <= paginationInfo.totalPages; i++) {
            paginationHtml += '<a href="#" onclick="History(' + i + ')">' + i + '</a>';
        }
    }
    // Display previous page link
    if (paginationInfo.currentPage > 1) {
        paginationHtml += '<a href="#" onclick="History( ' + (paginationInfo.currentPage - 1) + ')"><i class="fa fa-angle-double-left"></i></a>';
    }

    // Display next page link
    if (paginationInfo.currentPage < paginationInfo.totalPages) {
        paginationHtml += '<a href="#" onclick="History(' + (paginationInfo.currentPage + 1) + ')"><i class="fa fa-angle-double-right"></i></a>';
    }
